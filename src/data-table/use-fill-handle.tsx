...
export function useFillHandle({
  ...
  const anchorCellKey = useMemo(() => {
    if (!enabled || !selectedCell || !isColumnEditable(selectedCell.columnId)) return null;
    ...
    if (!selection) return `${selectedCell.rowId}-${selectedCell.columnId}`;
    const startIdx = rowIndexMap[selection.start.rowId] ?? -1;
    const endIdx = rowIndexMap[selection.end.rowId] ?? -1;
    if (startIdx === -1 || endIdx === -1) return `${selectedCell.rowId}-${selectedCell.columnId}`;
  ...
  }, [enabled, selectedCell, selection, rows, rowIndexMap, isColumnEditable, dragState]);

  const isAnchorCell = useCallback(
    (rowId: string, columnId: string) => anchorCellKey === `${rowId}-${columnId}`,
    [anchorCellKey],
  );
  ...
  const getFillBounds = (state: DragState): { min: number; max: number } | null => {
    const { selTopIdx, selBotIdx, hoverRowIdx } = state;
    if (hoverRowIdx > selBotIdx) return { min: selTopIdx + 1, max: hoverRowIdx };
    if (hoverRowIdx < selBotIdx) return { min: hoverRowIdx, max: selBotIdx - 1 };
    return null;
  };

  const isFillRangeCell = useCallback(
    (rowId: string, columnId: string) => {
      if (!dragState || columnId !== dragState.columnId) return false;
  ...
  const isFillSourceCell = useCallback(
    (rowId: string, columnId: string) => {
      ...
      if (hoverRowIdx > selBotIdx) return rowIdx === selTopIdx;
      if (hoverRowIdx < selBotIdx) return rowIdx === selBotIdx;
      return false;
    },
    [dragState],
  );
  ...
  const handleGlobalMouseMove = useCallback((e: MouseEvent) => {
    ...
    if (!td || td.dataset.columnId !== state.columnId) return;
    const rowIdx = rowIndexMapRef.current[td.dataset.rowId!] ?? -1;
    if (rowIdx === -1 || rowIdx === state.hoverRowIdx) return;
  ...
  const handleGlobalMouseUp = useCallback(() => {
    ...
    if (targetIndices.length > 0) {
      ...
      if (topRowId && botRowId && onFillComplete) {
        onFillComplete({ rowId: topRowId, columnId }, { rowId: botRowId, columnId });
      }
    }
  ...
  const fillHandleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      e.stopPropagation();
      e.preventDefault();
      if (!selectedCell || !enabled) return;
